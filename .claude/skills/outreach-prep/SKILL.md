---
name: outreach-prep
description: "Prepare and send the first-message Facebook outreach DM from Supabase dashboardkien_outreach, in strict outreach_order — agent pastes message1, clicks Send itself, and records the confirmed outcome. Processes in batches (default 20)."
---
...
## Agent prepares and sends — current model (2026-09-22, Kien's explicit decision)

**This reverses the 2026-09-18 "agent prepares, Kien clicks" model** — Kien
decided the manual-click step was too slow for real throughput and asked for
it removed. The agent now automates the whole loop: reading the queue,
opening the right post, verifying identity, opening Messenger, typing the
exact message, **and clicking Send itself** — no pause, no handoff.

This is the one write exception to "Facebook is read-only." Everything else
in CLAUDE.md #1 (no post/comment/like/join/submit form) still applies
unchanged — only the DM-send action itself is now agent-authorized.

Process in batches (default 20 people per run — Kien can ask for a different
batch size for a specific run), strictly by `outreach_order`, one send at a
time. Stop immediately (don't retry, don't guess) on: login prompt,
checkpoint, CAPTCHA, unusual-activity warning, or any unclear/ambiguous send
state — the "stop and report" discipline from the old model is unchanged,
only the click authority moved from Kien to the agent.
...
5. Click Send. Confirm from the UI (message appears in the thread as sent,
   composer clears) before treating it as sent — don't assume a click
   succeeded without visual confirmation.
...
Once the UI confirms the message sent:
...
2. Insert one `events` row with `event='outreach_dm_sent'`, `actor='agent'`
   (the agent performed the send itself, per the 2026-09-22 model), the post
...
- Batch size default is 20 sends per run — stop and report once reached,
  don't keep going past it without Kien starting a new run. This is the
  throttle that replaces the old per-message manual click.
