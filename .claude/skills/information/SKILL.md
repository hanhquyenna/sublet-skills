...
- **Cron đã bật (2026-09-16), khác mọi mô tả "chỉ chạy tay" ở nơi khác trong repo:** `data/config.yaml→hours` đổi thành 24/7 (`00:00`–`23:59`) theo yêu cầu Kien; launchd job `com.sublet.scrape_and_validate` (`ops/install_cron.sh` → `ops/run_scrape_and_validate.sh`) chạy mỗi 1200s, gọi `sublet-scrape-14-groups` rồi `validate-permalink` nối tiếp qua `claude -p` dùng **Claude in Chrome** (đã login Facebook lúc 2026-09-16, xem `ops_state.chrome_fb_login`). `ops/run_skill.sh` đã thêm `validate-permalink` vào whitelist và đọc `hours` từ config thay vì hardcode 8–23. Kiểm tra job đang chạy: `launchctl list | grep com.sublet`; log tại `ops/logs/`. Gỡ: `zsh ops/install_cron.sh uninstall`.
