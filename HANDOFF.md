...
- Code + schema xong. Supabase canonical là project ref `cteunhuxrghpozwbnehh`; `scripts/db.py` dùng `~/.sublet-skills.env` + REST RPC `sublet_exec`. DB hiện có 92 `sublet_groups`, 0 listings, 0 seekers, 0 ops_state. 20 group đã được kiểm tra trực tiếp trong panel; current DB ghi nhận 16 `joined=true` và 76 `joined=false` do Kien thao tác, agent không join. 72 group còn lại chưa verify trực tiếp, gồm 24 candidate từ web và 39 candidate mới từ Facebook panel. Config `your_first_name=Kien`; `email.imap_user` và `seeker_form.url` còn trống. Chưa có cron.
- Kiến trúc: KNOW (sublet-groups) → CAPTURE (sublet-scan qua ChatGPT browser panel / sublet-email qua IMAP, lưu thô kind=null) → ANALYZE (intent-analyze theo docs/intent-logic.md: kind, subtype, poster_type, fields, scam_score, deal_score, confidence) → MATCH (scripts/match.py deterministic) → VOICE/RUN (partner-voice; sublet-draft; inbox-triage; viewing-coordinate; sublet-followup). Ops: onboarding, sublet-report, sublet-diagnose, seeker-intake.
