...
| R25 | 14-day capture is one group at a time; update DB after each batch. Complete = past boundary AND unresolved-link cards ≤5 or ≤10% of verified (nới lỏng 2026-09-16); below that stays incomplete. Unresolved-link cards still require full raw capture (`capture_unresolved` with complete text/media), never skipped for content | `sublet-scrape-14-groups`, capture contract | `posts_14d_complete` only true after boundary + threshold met; every `capture_unresolved` event has non-empty raw text | enforce |
